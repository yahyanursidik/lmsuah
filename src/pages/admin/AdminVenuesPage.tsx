    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-slate-900">Pengelolaan Lokasi Majelis</h1>
          <p className="text-xs text-slate-500">Satu data lokasi untuk jadwal admin, website utama, dan portal peserta.</p>
        </div>
      ...
      <Card className="bg-white border-slate-200 text-slate-900">
        <CardHeader className="border-b border-slate-200">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-slate-900 text-base">Daftar Lokasi Majelis</CardTitle>
              <CardDescription className="text-slate-500">Cari, periksa status, lalu buka tampilan publik.</CardDescription>
            </div>
            <Button
              size="sm"
              variant="ghost"
        ...
        <CardContent className="p-4 sm:p-6">
          <div className="mb-5 flex flex-col gap-3 sm:flex-row"><label className="relative flex-1"><span className="sr-only">Cari lokasi</span><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari nama, alamat, atau kota…" className="min-h-11 w-full rounded-lg border border-slate-200 bg-slate-50 pl-10 pr-3 text-sm text-slate-900 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-500/20" /></label><select aria-label="Filter status lokasi" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as typeof statusFilter)} className="min-h-11 rounded-lg border border-slate-200 bg-slate-50 px-3 text-sm text-slate-900 outline-none focus:border-emerald-400"><option value="all">Semua status</option><option value="active">Aktif</option><option value="inactive">Nonaktif</option></select></div>
          {isLoading ? (
            ...
            <>
                ...
                <table className="w-full text-left text-xs text-slate-700">
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {filteredVenues.map((v: VenueItem) => (
                      <tr key={v.id} className="hover:bg-slate-50 hover:bg-slate-100">
                        ...
                        <td className="p-3 text-slate-700">{v.city}</td>
                        <td className="p-3">
                          {v.googleMapsUrl ? (
                            <a
              ...
              <div className="block md:hidden space-y-4">
                {filteredVenues.map((v: VenueItem) => (
                  <div key={v.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-3">
                    ...
                    <p className="text-xs text-slate-500">{v.address}, {v.city}</p>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 overflow-y-auto">
          <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 space-y-5 shadow-2xl my-8">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <h3 className="text-lg font-bold text-slate-900">
                {editingVenue ? 'Edit Lokasi Majelis' : 'Tambah Lokasi Baru'}
              </h3>
              <button onClick={() => setIsModalOpen(false)} className="text-slate-500 hover:text-slate-900 p-1">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-700">Nama Masjid / Majelis *</label>
                <input
                  type="text"
                  required
                  value={formData.name}
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-900 focus:border-emerald-400 focus:outline-none min-h-[44px]"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-900 focus:border-emerald-400 focus:outline-none"
                />
              </div>
              ...
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-700">Tautan Google Maps</label>
                <input
                  type="url"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-slate-700">Kapasitas Jamaah</label>
                  <input
                    type="text"
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-200">
                <Button
                  ...
                  className="text-slate-500 hover:text-slate-900 min-h-[44px]"
                >
                  Batal
                </Button>
                <Button
                  type="submit"
                  disabled={isSaving}
                  className="bg-emerald-600 hover:bg-emerald-700 text-slate-900 min-h-[44px] px-5"
                >
